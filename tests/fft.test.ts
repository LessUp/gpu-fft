...
import { cpuFFT, cpuIFFT, validateFFTInput } from '../src/utils/cpu-fft';
import { naiveDFT, interleavedToComplex, complexToInterleaved, complexApproxEqual } from '../src/utils/complex';
...
describe('FFT Correctness', () => {
    ...
    it('property: any non-power-of-2 size throws error', () => {
      fc.assert(
        fc.property(fc.integer({ min: 2, max: 1000 }), (n: number) => {
          ...
          if (isPowerOf2) return true; // Skip powers of 2
  ...
  describe('FFT Correctness - Additional Tests', () => {
    it('FFT preserves Parseval\'s theorem (energy conservation)', () => {
      fc.assert(
        fc.property(fc.integer({ min: 2, max: 6 }).map(n => Math.pow(2, n)), (size: number) => {
          const input = new Float32Array(size * 2);
          for (let i = 0; i < input.length; i++) {

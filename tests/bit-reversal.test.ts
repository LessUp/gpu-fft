...
import { bitReverse, log2, isPowerOf2, bitReversalPermutation } from '../src/utils/bit-reversal';
      ...
      fc.assert(
        fc.property(powerOf2SizeArb, (size: number) => {
          ...
          for (let i = 0; i < size; i++) {
            ...
            if (output[i * 2] !== input[j * 2] || output[i * 2 + 1] !== input[j * 2 + 1]) {
              return false;
            }
          }
          return true;

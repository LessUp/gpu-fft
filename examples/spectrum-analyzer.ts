...
async function main(): Promise<void> {
    ...
    try {
        ...
        console.log(`Creating spectrum analyzer (FFT size: ${fftSize}, sample rate: ${sampleRate} Hz)...`);
        ...
        console.log(`Expected: ${testFreq1} Hz, Detected: ${detectedFreq1.toFixed(1)} Hz, Error: ${freqError1.toFixed(1)} Hz`);
        ...
        const signal2 = generateMultiFrequencySignal(sampleRate, chordFreqs, chordAmps, fftSize / sampleRate);
        ...
        for (const expectedFreq of chordFreqs) {
            const found = peaks2.some(p => Math.abs(p.frequency - expectedFreq) < sampleRate / fftSize * 2);
            if (!found) {
